...
// static CryptoInitWrapper cryptoInitWrapper;
Client client;
// std::cout<<"cryptoInitWrapper ok"<<endl;
// sse::sophos::SophosClient Sophos_client;
// std::cout<<"Sophos_client ok"<<std::endl;
 ...
 */
int PQsendQuery(PGconn* conn, const char* query)//改这
{   
    sseflag.resetSSEflags();
    string qs_cpy(query);
    string qs_ori(query);
    transform(qs_cpy.begin(), qs_cpy.end(), qs_cpy.begin(), (int(*)(int))tolower);


    string qs_seven(qs_cpy,0,7);
    string qsfides_11(qs_cpy, 0, 11);
    string qsfides_12(qs_cpy, 0, 12);
    string temp;
    if(qs_seven=="*insert"||qs_seven=="*select")
    {
        //插入时，原语句是 *Insert into table values('pt');   表结构是cd  ct密文(加密列)
        //这里要加上根据明文在本地表中计算pos，并改写sql  目标"Insert into table values(pos=15,'pt');"  交给密态功能去加密
        if (qs_seven=="*insert")//要改写
        {
            int qs_pre_pos = qs_cpy.find("(");
	        string qs_pre(qs_cpy, 1, qs_pre_pos);//Insert into table values(
            int pt_begin=qs_cpy.find("'", qs_pre_pos);
            int pt_end=qs_cpy.find("'", pt_begin+1);
            string plaintext(qs_cpy, pt_begin+1, pt_end - pt_begin-1);
            int pos=client.insert_table(plaintext);
            temp=qs_pre + "pos=" + to_string(pos) + "," + "'" + plaintext + "');";
        }

        //搜索时，原语句是"*Select name,card from talbe where plaintext>'abc' and plaintext<='bcd';"
        //where pt>'ptmin' and pt<='ptmax'要改成pos> and pos<=   目标"Select name,card from talbe where pos>8 and pos<=15;"
        if(qs_seven=="*select")
        {
            string qs_cpy_lower(qs_cpy);
		    transform(qs_cpy_lower.begin(), qs_cpy_lower.end(), qs_cpy_lower.begin(), (int(*)(int))tolower);
		    int qs_pre_pos = qs_cpy_lower.find("where");
		    string qs_pre(qs_cpy, 1, qs_pre_pos + 5);//Select name,card from talbe where //where后有个空格
		    //cout <<qs_pre_pos <<"   "<<qs_pre << endl;
		    //cout << qs_cpy << endl;
		    int ptmin_begin = qs_cpy.find(">'", qs_pre_pos);
		    int ptmin_end = qs_cpy.find("'",ptmin_begin+2);
		    string ptmin(qs_cpy, ptmin_begin + 2, ptmin_end - ptmin_begin - 2);
		    int ptmax_begin = qs_cpy.find("<='", qs_pre_pos);
		    int ptmax_end = qs_cpy.find("'", ptmax_begin + 3);
		    string ptmax(qs_cpy, ptmax_begin + 3, ptmax_end - ptmax_begin - 3);
            temp=qs_pre+"pos>"+ to_string(client.search_left(ptmin)) +" pos<="+ to_string(client.search_right(ptmax)) +";";
        }
    }
    if(qsfides_12 == "fides_insert" || qsfides_12 == "fides_select")
    {
	    //SophosServer server;
        if (qsfides_12 == "fides_insert")//要改写  fides_insert into Fides_datatable values('1','aaa','aaa is aaa');
		{
			sse::sophos::new_UpdateRequest req;
            vector<sse::sophos::new_UpdateRequest> reqs;
            string qsfides_15(qs_cpy, 0, 15);
            bool ind=(qsfides_15=="fides_insertind");//just update ind|op

            vector<pair<string,string>>fides_index;// keyword-index
            vector<tuple<string,string,string>>fides_data;
            int pos = qs_cpy.find("(");
		    string qs_pre(qs_cpy, 6, pos-6);//Insert into fides_datatable values
            while (pos != string::npos) {
                // 找到左括号后的第一个单引号的位置
                size_t start = qs_cpy.find("'", pos);
                if (start != string::npos) {
                    // 找到第一个单引号后的第二个单引号的位置，表示值的结束
                    size_t end = qs_cpy.find("'", start + 1);
                    if (end != string::npos) {
                        // 提取值ind
                        string first = qs_ori.substr(start + 1, end - start - 1);
                        start = qs_cpy.find("'", end + 1);
                        if (start != std::string::npos) {
                            end = qs_cpy.find("'", start + 1);
                            if (end != std::string::npos) {
                                // 提取值keyword
                                string second = qs_ori.substr(start + 1, end - start - 1);
                                if(!ind)
                                {
                                    start = qs_cpy.find("'", end + 1);
                                    if (start != std::string::npos) {
                                        ...
                                        if (end != std::string::npos) {
                                            // 提取值value
                                            string third = qs_ori.substr(start + 1, end - start - 1);
                                            fides_index.push_back({second, first});
                                            fides_data.push_back(make_tuple(first,second,third));
                                        }
                                    }
                                }
                                else
                ...
                // 继续查找下一个值对的位置
                pos = qs_cpy.find("(", pos + 1);
            }
            std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
            int size=fides_index.size();
			for(int i=0;i<size;i++)
            {
                req=Sophos_client->insertion_request(fides_index[i].first,fides_index[i].second, i==size-1);
                reqs.push_back(req);
            }
            std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> time_span = t2 - t1;
            std::cout << "client It took " << time_span.count() << " milliseconds."<<std::endl;
		    // int keyword_begin = qs_cpy.find("'", qs_pre_pos);
		    // int keyword_end = qs_cpy.find("'", keyword_begin + 1);			    
            // string keyword(qs_cpy, keyword_begin + 1, keyword_end - keyword_begin - 1);
		    // int index_begin = qs_cpy.find("'", keyword_end+1);
		    // int index_end = qs_cpy.find("'", index_begin + 1);
		    // string index(qs_cpy, index_begin + 1, index_end - index_begin - 1);
	    	// req = Sophos_client.insertion_request(keyword, index);
            ...
            if(!ind)
            {
                ...
                string ut_array="['";
                //fides_data和reqs数量一样
                for (int i = 0; i < fides_data.size(); i++)
	            {
		            if (i != fides_data.size() - 1)
		            {
			            ind_array=ind_array+get<0>(fides_data[i])+"', '";
			            keyword_array= keyword_array + get<1>(fides_data[i]) + "', '";
			            values_array= values_array + get<2>(fides_data[i]) + "', '";
                        ut_array= ut_array + reqs[i].token + "', '";
		            }
		            else
		            {
			            ind_array = ind_array + get<0>(fides_data[i]) + "'];";
...
DECLARE
    ...
    end_time TIMESTAMP;
    duration INTERVAL;
    ind_array TEXT[] := ARRAY)" +ind_array+
	R"(
    keyword_array TEXT[] := ARRAY)"+keyword_array+
    R"(
    value_array TEXT[] := ARRAY)"+values_array+
	R"(
    ut_array TEXT[] := ARRAY)"+ut_array+
	R"(
BEGIN
    start_time := clock_timestamp();
    PERFORM fides_insert_bulk(ind_array, keyword_array, value_array, ut_array);
    end_time := clock_timestamp();
    duration := end_time - start_time;
    RAISE NOTICE 'fides_insert_bulk execution time: %', duration;
END $$;
)";
            }
            else
            {
                ...
                for(int i=0;i<reqs.size();i++)
                {
                    if(i!=reqs.size()-1)
                    {
                        temp = temp + "('" + reqs[i].token +"','" + reqs[i].index + "'),";
                    }
                    else
                    {
                        temp = temp + "('" + reqs[i].token +"','" + reqs[i].index + "');";
		...
		{
            string qsfides_15(qs_cpy, 0, 15);
            bool indexupdate=(qsfides_15=="fides_selectind");//just update ind|op
            if(indexupdate)
            {
                //重插入
                sse::sophos::new_UpdateRequest select_req;
                vector<sse::sophos::new_UpdateRequest> select_reqs;

                vector<string>fides_ind;
                size_t start = qs_cpy.find("*");
                size_t end = qs_cpy.find("*", start + 1);
                string keyword = qs_ori.substr(start + 1, end - start - 1);
                int pos = qs_cpy.find("(");
                while (pos != string::npos) {
                    // 找到左括号后的第一个单引号的位置
                    size_t start = qs_cpy.find("'", pos);
                    if (start != string::npos) {
                        ...
                        if (end != string::npos&&end-start>1) {
                            // 提取值ind
                            string first = qs_ori.substr(start + 1, end - start - 1);
                            fides_ind.push_back(first);
                        }
                    }
                    pos = qs_cpy.find(",", pos + 1);
                }
                int size=fides_ind.size();
                for(int i=0;i<fides_ind.size();i++)
                {
                    select_req=Sophos_client->insertion_request(keyword,fides_ind[i], i==size-1);
                    select_reqs.push_back(select_req);
                }
                string select_ut_array="ARRAY['";
                string select_ind_array="ARRAY['";
                //fides_data和reqs数量一样
                for (int i = 0; i < fides_ind.size(); i++)
	            {
		            if (i != fides_ind.size() - 1)
		            {
                        select_ut_array= select_ut_array + select_reqs[i].token + "', '";
			            select_ind_array=select_ind_array+fides_ind[i]+"', '";
		            }
		            else
		            {
                        select_ut_array = select_ut_array + select_reqs[i].token + "']";
                ...
                if(fides_ind.size()==0)
                {
                    select_ut_array = select_ut_array + "']";
			        select_ind_array = select_ind_array + "']";
                }
                temp="select * from fides_select("+select_ut_array+", "+ select_ind_array+");";
            }
            else
            {
                sseflag.fides=true;//fang dao sou suo chu
	    	    sse::sophos::new_SearchRequest sreq;
		        int keyword_begin = qs_cpy.find("'");
			    int keyword_end = qs_cpy.find("'", keyword_begin + 1);
    		    string keyword(qs_ori, keyword_begin + 1, keyword_end - keyword_begin - 1);
        	    sreq = Sophos_client->search_request(keyword);

		        // temp = "fides_select * from fides_indextable where token=" + std::string(sreq.token.begin(), sreq.token.end()) + " and derivation_key="  +sreq.derivation_key + " and add_count=" + to_string(sreq.add_count)+";";
                // temp = "fides_select * from fides_indextable where token=" + sse::sophos::base64_encode(sreq.token) + " and derivation_key="  +sse::sophos::base64_encode(sreq.derivation_key) + " and add_count=" + to_string(sreq.add_count)+";";
                
                // temp = "fides_select * from fides_indextable where token=" + sreq.token + " and derivation_key="  +sreq.derivation_key + " and add_count=" + to_string(sreq.add_count)+";";//zhege
                temp = "fides_select * from athletes_index where token=" + sreq.token + " and derivation_key="  +sreq.derivation_key + " and add_count=" + to_string(sreq.add_count)+";";
            }
	    }
	}
    if(qsfides_12 == "fides_delete")//Fides_delete from fides_datatable where ind='x' and keyword='xx';
    {
        string ind,keyword;
        size_t ind_pos = qs_cpy.find("ind");
        size_t keyword_pos = qs_cpy.find("keyword");
        // sse::sophos::UpdateRequest del_req;
        sse::sophos:: new_UpdateRequest del_req;
        if (ind_pos != string::npos && keyword_pos != string::npos)
        {
            // 提取值ind
            size_t start = qs_cpy.find("'",ind_pos+1);
            if (start != string::npos) 
            {
                // 找到第一个单引号后的第二个单引号的位置，表示值的结束
                size_t end = qs_cpy.find("'", start + 1);
                if (end != string::npos) 
                {
                    ind = qs_ori.substr(start + 1, end - start - 1);
                }
            }
            // 提取值keyword
            start = qs_cpy.find("'",keyword_pos+1);
            if (start != string::npos) 
            {
                // 找到第一个单引号后的第二个单引号的位置，表示值的结束
                size_t end = qs_cpy.find("'", start + 1);
                if (end != string::npos)
                 {
                    keyword = qs_ori.substr(start + 1, end - start - 1);
                }
            }
        }
        del_req=Sophos_client->insertion_request(keyword,ind);

        // temp="insert into fides_indextable values('"+std::string(del_req.token.begin(), del_req.token.end())+"', '"+ind+"|del');";

        // temp="insert into fides_indextable values('"+del_req.token+"', '"+ind+"|del');";
        temp="insert into athletes_index values('"+del_req.token+"', '"+ind+"|del');";
    }

    if(qsfides_11 == "fides_setup")
    {
        // temp="fides_select * from fides_indextable where ut='setup' and pk="+Sophos_client->public_key()+";";
        temp="fides_select * from athletes_index where ut='setup' and pk="+Sophos_client->public_key()+";";
    }

    if(qs_seven!="*select"&&qs_seven!="*insert"&&qsfides_12!="fides_insert"&&qsfides_12!="fides_select"&&qsfides_12!="fides_delete"&&qsfides_11!="fides_setup")
    {
        temp = query;
    }
    const char* query_changed = temp.c_str();
    query=query_changed;
    ...
    if (conn->client_logic->enable_client_encryption) {//开启了密态
#if ((!defined(ENABLE_MULTIPLE_NODES)) && (!defined(ENABLE_PRIVATEGAUSS)))
        ...
        if (!conn->client_logic->disable_once) {
            ...
            if (!clientLogicRet)
                return 0;
            query = statementData.params.adjusted_query;//const？？在这改成的加密的内容，所以这之后insert要改成 （pos= ，ct）
        } else {
        ...
        if (temp_query != NULL && ((strcasestr(temp_query, global_setting_str) != NULL) ||
            ...
            (strcasestr(temp_query, client_logic_str) != NULL))) {//不走
            free(temp_query);
            temp_query = NULL;
    ...
    if (pqPutMsgStart('Q', false, conn) < 0 || pqPuts(query, conn) < 0 || pqPutMsgEnd(conn) < 0) {//不走
...
//bu zai zhe gai   common.cpp
    // switch (sseflag.bool2count())
    // {
    // case 1://fides
    //     // guolv
    //     vector<string> records;
    //     for(int i=0;i<res->ntups;i++)
    //     {
    //         // records.push_back(string(*(*(res->tuples+i)+1)));
    //         records.push_back(string(res->tuples[i][1].value));
    //     }
    //     filterRecords(records, res->ntups);

    //     //gouzao sql
    //     string fides_temp="SELECT * from Fides_datatable WHERE ind IN ('";
    //     for(int i=0;i<records.size();i++)
    //     {
    //         if(i!=records.size()-1)
    //         {
    //             fides_temp=fides_temp+records[i]+"','";
    //         }
    //         else
    //         {
    //             fides_temp=fides_temp+records[i]+"');";
    //         }
    //     }
    //     const char* fides_query2=fides_temp.c_str();

        
    //     //result=PQexec(conn, fides_query2);
    //     //sendQuery   res=finish

    //     // if(!PQsendQuery(conn,fides_query2))
    //     // {
    //     //     return NULL;
    //     // }
