 ...
 */
int PQsendQuery(PGconn* conn, const char* query)//改这好insert要改成 (pos=，pt) 原语句应该是 (pt)
{   
    string qs_cpy(query);
    string small_qs_cpy(query);
    transform(small_qs_cpy.begin(), small_qs_cpy.end(), small_qs_cpy.begin(), (int(*)(int))tolower);
    string qs_seven(small_qs_cpy,0,7);
    string qsmitra_12(small_qs_cpy,0,12);
    string temp;
    if(qs_seven=="*insert"||qs_seven=="*select")
    {
        ...
        if (qs_seven=="*insert")//要改写
        {
	        ...
	        string qs_pre(qs_cpy, 0, qs_pre_pos+1);//Insert into table values(
        ...
        if(qs_seven=="*select")
        {
		    ...
		    string qs_pre(qs_cpy, 0, qs_pre_pos + 6);//Select name,card from talbe where //where后有个空格
    ...
    if(qsmitra_12=="mitra_insert" || qsmitra_12=="mitra_select")
    {
        if(qsmitra_12=="mitra_insert")
        {
            prf_type addr;
            prf_type val;
            vector<string>addrs;
            vector<string>vals;
            string qsmitra_15(small_qs_cpy, 0, 15);
            bool ind=(qsmitra_15=="mitra_insertind");//just update ind|op
            vector<pair<int,string>>mitra_index;// index-keyword
            vector<tuple<int,string,string>>mitra_data;
            int pos = qs_cpy.find("(");
            while (pos != string::npos) {
                ...
                if (start != string::npos) {
                    ...
                    if (end != string::npos) {
                        // 提取值ind
                        int index = stoi(qs_cpy.substr(start + 1, end - start - 1));
                        start = qs_cpy.find("'", end + 1);
                        if (start != std::string::npos) {
                            end = qs_cpy.find("'", start + 1);
                            if (end != std::string::npos) {
                                // 提取值keyword
                                string keyword = qs_cpy.substr(start + 1, end - start - 1);
                                if(!ind)
                                {
                                    start = qs_cpy.find("'", end + 1);
                                    if (start != std::string::npos) {
                                        ...
                                        if (end != std::string::npos) {
                                            // 提取值value
                                            string data = qs_cpy.substr(start + 1, end - start - 1);
                                            mitra_index.push_back({index, keyword});
                                            mitra_data.push_back(make_tuple(index,keyword,data));
                                        }
                                    }
                                }
                                else
            ...
            int size=mitra_index.size();
			for(int i=0;i<size;i++)
            {
                Mitra_client->updateRequest(OP::INS, mitra_index[i].second, mitra_index[i].first, addr, val);
                addrs.push_back(base64_encode(string(addr.begin(), addr.end())));
                vals.push_back(base64_encode(string(val.begin(), val.end())));
            }
            ...
            if(!ind)
            {
                ...
                string addr_array="['";
                string val_array="['";
                //mitra_data和addts数量一样
                for (int i = 0; i < mitra_data.size(); i++)
	            {
		            if (i != mitra_data.size() - 1)
		            {
			            ind_array=ind_array+to_string(get<0>(mitra_data[i]))+"', '";
			            keyword_array= keyword_array + get<1>(mitra_data[i]) + "', '";
			            values_array= values_array + get<2>(mitra_data[i]) + "', '";
                        addr_array=addr_array+addrs[i]+"', '";
                        val_array= val_array + vals[i] + "', '";
		            }
		            else
		            {
			            ind_array = ind_array + to_string(get<0>(mitra_data[i]))+ "'];";
...
DECLARE
	...
	R"(
    addr_array TEXT[] := ARRAY)"+addr_array+
	R"(
    val_array TEXT[] := ARRAY)"+val_array+
	R"(
BEGIN
    start_time := clock_timestamp();
    PERFORM mitra_insert_bulk(ind_array, keyword_array, value_array, addr_array, val_array);
    end_time := clock_timestamp();
    duration := end_time - start_time;
    RAISE NOTICE 'mitra_insert_bulk execution time: %', duration;
END $$;
)";
            }

        } 
        if(qsmitra_12=="mitra_select")
        {
            vector<string>list;
            string qsmitra_15(qs_cpy, 0, 15);
            bool indexupdate=(qsmitra_15=="mitra_selectind");//just update ind
            if(indexupdate)
            {
            ...
            else
            {
        	    ...
        	    Mitra_client->searchRequest(keyword,list);
                // temp = "select * from Mitra_indextable where addr in ('";
                temp = "delete from Mitra_indextable where addr in ('";
                int lsize=list.size();
                ...
                if(lsize>0)
                {
                    temp=temp+list[lsize-1]+"') RETURNING *;";
                }
                else
                {
                    // temp=temp+"');";
    ...
    if(qsmitra_12=="mitra_delete")//mitra_delete from fides_datatable where ind='x' and keyword='xx';
    {
        prf_type addr;
        prf_type val;
        memset(addr.data(), 0, AES_KEY_SIZE);
        memset(val.data(), 0, AES_KEY_SIZE);
        string ind,keyword;
        size_t ind_pos = small_qs_cpy.find("ind");
        size_t keyword_pos = small_qs_cpy.find("keyword");
        if (ind_pos != string::npos && keyword_pos != string::npos)
        {
            ...
            if (start != string::npos) 
            {
                ...
                if (end != string::npos) 
                {
                    ind = qs_cpy.substr(start + 1, end - start - 1);
                }
            }
            // 提取值keyword
            ...
            if (start != string::npos) 
            {
                ...
                if (end != string::npos)
                 {
                    keyword = qs_cpy.substr(start + 1, end - start - 1);
                }
            }
        }
        Mitra_client->updateRequest(OP::DEL, keyword, stoi(ind), addr, val);
        temp="insert into mitra_indextable values('"+base64_encode(string(addr.begin(), addr.end()))+"', '"+base64_encode(string(val.begin(), val.end()))+"');";

    }
    if(qs_seven!="*insert"&&qs_seven!="*insert"&&qsmitra_12!="mitra_insert"&&qsmitra_12!="mitra_select"&&qsmitra_12!="mitra_delete")
    {
        temp = query;
    }
