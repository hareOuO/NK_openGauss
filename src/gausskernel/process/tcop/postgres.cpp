...
static void exec_simple_query(const char* query_string, MessageType messageType, StringInfo msg = NULL)
{
    ...
    if(qs_six=="insert"||qs_six=="select")
    {
        ...
        {   
            ...
            int fpos_begin = qs_cpy.find("pos=");
            if (fpos_begin != qs_cpy.npos)//如果格式符合,这里改写query_string   pos  ct 改成插入 cd  'ct'
            {
                //前缀
	            int qs_pre_pos = qs_cpy.find("(");
	            string qs_pre(qs_cpy, 0, qs_pre_pos+1);//qs_pre 是Insert into talbe values(
                int fpos_end = qs_cpy.find(",");
                string spos(qs_cpy, fpos_begin + 4, fpos_end - fpos_begin - 4);
	            int pos = atoi(spos.c_str());
                int ct_begin = fpos_end + 2;
	            int ct_end = qs_cpy.find("'", ct_begin);
                string cipher(qs_cpy, ct_begin, ct_end - ct_begin);
                //long long start_update = -1;
                //long end_update = -1;
                //update.clear();
                long long re = root->insert(pos, cipher);
                temp=qs_pre + to_string(re) + "," + "'" + cipher + "');";
                if(re==0)
                {
                    LeafNode *leftchild=(LeafNode*)root->getminLeafNode();
                    for(leftchild;leftchild->right_bro!=NULL;leftchild=leftchild->right_bro)//除了最后一个叶节点的遍历
                    {
                        for(int j=0;j<leftchild->encoding.size();j++)
                        {
                            if((leftchild->encoding[j] >= lower_bound() && leftchild->encoding[j] < upper_bound()) || leftchild->encoding[j] == 0)
                            temp+"update table set encoding = "+to_string(get_update(leftchild->cipher[j]))+"where encoding="+to_string(leftchild->encoding[j])+";";
                        }//确定性加密会导致相同明文相同密文有相同的encoding，原因是在触发get_update时会相同的密文改成一样的encoding
                    }
                    for(int j=0;j<leftchild->encoding.size();j++)
                    {
                        if((leftchild->encoding[j] >= lower_bound() && leftchild->encoding[j] < upper_bound()) || leftchild->encoding[j] == 0)
                        temp+"update table set encoding = "+to_string(get_update(leftchild->cipher[j]))+" where ciphertext="+ leftchild->cipher[j] +";";
                    }//试试改的行不行
                }
            }
            else
        ...
        //query_string是这种"Select name,card from talbe where pos>8 and pos<=15;"
        //搜索时
        if (qs_six == "select")
	    {
		    int lpos_begin = qs_cpy.find("pos>");
		    if (lpos_begin != qs_cpy.npos)//如果格式符合,这里改写query_string   pos>  pos<= 改成encoding> encoding<=
		    {
			    //前缀
			    string qs_pre(qs_cpy, 0, lpos_begin);
			    //左边界
			    int lpos_end = qs_cpy.find(" ",lpos_begin);
			    string lspos(qs_cpy, lpos_begin + 4, lpos_end - lpos_begin - 4);
			    int lpos = atoi(lspos.c_str());
			    //右边界
			    int rpos_begin = qs_cpy.find("pos<=");
			    int rpos_end = qs_cpy.find(";", rpos_begin);
			    string rspos(qs_cpy, rpos_begin + 5, rpos_end - rpos_begin - 5);
			    int rpos = atoi(rspos.c_str());
			    //cout << rpos << endl;
			    long long lcd = root->search(lpos);
			    long long rcd = root->search(rpos);
			    temp = qs_pre + "encoding>" + to_string(lcd)+" and encoding<=" + to_string(rcd) + ";";
		    }
		    else
			    temp = query_string;
	    }
    ...
    if(qs_six=="fides_")
    {
        int init = qs_cpy.find("setup");
        if(init != qs_cpy.npos)
        {
            std::string key_str(query_string);
            // 定义要查找的公钥标志
            std::string key_start = "-----BEGIN PUBLIC KEY-----";
            std::string key_end = "-----END PUBLIC KEY-----";

            // 查找公钥的起始位置
            std::size_t start_pos = key_str.find(key_start);
            // if (start_pos == std::string::npos) {
        //         std::cerr << "Public key start not found." << std::endl;
        //         return 1;
            // }

            // 查找公钥的结束位置
            std::size_t end_pos = key_str.find(key_end, start_pos);
        //     if (end_pos == std::string::npos) {
        //         std::cerr << "Public key end not found." << std::endl;
        //         return 1;
        //     }

            // 提取公钥部分
            end_pos += key_end.length();
            std::string public_key = key_str.substr(start_pos, end_pos - start_pos);
            sse::sophos::init_SophosServer_key(public_key);

            Sophos_server=sse::sophos::init_SophosServer();
            // temp="select * from fides_indextable  \\g /dev/null";
            temp="SELECT 'ok' AS Fides_setup;";
        }
        else
        {
            if(Sophos_server==nullptr)
            {
                temp="SELECT 'no(need to run ''fides_setup'')' AS Fides_setup;";
            }
            else
            {
                string fides_str(query_string);//用原始大小写
                sse::sophos::new_server_SearchRequest sreq;
                int token_begin = qs_cpy.find("token=");
		        int token_end = qs_cpy.find(" ", token_begin + 1);
                string token(fides_str, token_begin + 6, token_end - token_begin - 6);
                sreq.token=sse::sophos::base64_decode(token);

                int derivation_key_begin = qs_cpy.find("derivation_key=",token_end+1);
		        int derivation_key_end = qs_cpy.find(" ", derivation_key_begin + 1);
		        string derivation_key(fides_str, derivation_key_begin + 15, derivation_key_end - derivation_key_begin - 15);
                sreq.derivation_key=sse::sophos::base64_decode(derivation_key);

                int add_count_begin = qs_cpy.find("add_count=", derivation_key_end + 1);;
		        int add_count_end = qs_cpy.find(";", add_count_begin);
		        string add_count(qs_cpy, add_count_begin + 10, add_count_end - add_count_begin - 10);
                sreq.add_count=stoul(add_count);

                temp=Sophos_server->search(sreq);
            }
            
        }
    }
	if(qs_six!="insert"&&qs_six!="select"&&qs_six!="fides_")
	{
		temp = query_string;
	}
    const char* query_string_changed = temp.c_str();
    query_string=query_string_changed;
     ...
     */
    if (HYBRID_MESSAGE != messageType) {//rangequery  走这
    ...
    if (check_log_statement(parsetree_list)) {//bu走
     ...
     */
    if (!t_thrd.postgres_cxt.password_changed) {//bu走
    ...
    if (current_user != BOOTSTRAP_SUPERUSERID) {//bu走
    ...
    if (t_thrd.wlm_cxt.collect_info->sdetail.statement != NULL && list_length(parsetree_list) > 1) {//bu走
        is_multi_query_text = true;
    }

    /* just for cooperation analysis. */
    if (IS_PGXC_COORDINATOR && u_sess->pgxc_cxt.is_gc_fdw) {//bu走
    ...
    foreach (parsetree_item, parsetree_list) {
        ...
        if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) && PointerIsValid(query_string_locationlist) &&
            list_length(query_string_locationlist) > 1) {//bu走
            oldcontext = MemoryContextSwitchTo(t_thrd.mem_cxt.msg_mem_cxt);
            is_multistmt = true;
            query_string_single =
...
#ifdef ENABLE_MULTIPLE_NODES//bu走
         ...
         * state, but not many...)
         */
        if (IsAbortedTransactionBlockState() && !IsTransactionExitStmt(parsetree))//bu走
            ereport(ERROR,
                (errcode(ERRCODE_IN_FAILED_SQL_TRANSACTION),
                    errmsg("current transaction is aborted, "
                        "commands ignored until end of transaction block, firstChar[%c]",
        ...
        /* Try using light proxy to execute query *///bu走
        ...
        if (was_logged != false && randomPlanInfo != NULL) {//bu走
        ...
        if (IsA(parsetree, VacuumStmt)) {//bu走
            /* copy the parsetree */
        ...
        if ((IS_PGXC_DATANODE || IsConnFromCoord()) && IsA(parsetree, ReindexStmt) &&
            ((ReindexStmt*)parsetree)->kind == OBJECT_DATABASE && IsPostmasterEnvironment)//bu走
            SetForceXidFromGTM(true);
#endif
        /* SQL bypass */
        if (runOpfusionCheck) {
            ...
            if (opFusionObj != NULL) {//bu走，insert走了
        ...
        /*我们不需要将任何内容复制到门户中，
        因为我们在这里传递的所有内容都在t_thrd.mem_cxt中。
        Msg_mem_cxt，它将比门户存在的时间更长。如果我们收到一个hybridmessage，
        我们将sql_query_string作为原始查询字符串发送给PortalDefineQuery。
         * We don't have to copy anything into the portal, because everything
         * we are passing here is in t_thrd.mem_cxt.msg_mem_cxt, which will outlive the
         ...
         */
        if (HYBRID_MESSAGE != messageType) {//走这
            if (is_multistmt && (IsConnFromApp() || IsConnFromInternalTool())) {//走这
        ...
        if (ENABLE_WORKLOAD_CONTROL && IS_PGXC_COORDINATOR && is_multi_query_text) {//bu走
         ...
         */
        format = 0; /* TEXT is default */
        if (IsA(parsetree, FetchStmt)) {//bu走
        ...
        if (u_sess->attr.attr_resource.use_workload_manager && g_instance.wlm_cxt->gscgroup_init_done &&
            !IsAbortedTransactionBlockState()) {//bu走
        ...
        if (IsA(parsetree, TransactionStmt)) {
             ...
             */
            finish_xact_command();
        } else if (lnext(parsetree_item) == NULL) {//走这
            /*
     ...
     */
    if (parsetree_list == NULL)//bu走
        NullCommand(dest);

    /* release global active counts */
    if (ENABLE_WORKLOAD_CONTROL) {//走这
        if (g_instance.wlm_cxt->dynamic_workload_inited) {//bu走
    ...
    if (save_log_statement_stats)//bu走
        ShowUsage("QUERY STATISTICS");
...
int PostgresMain(int argc, char* argv[], const char* dbname, const char* username)
{
    ...
    //     }
    // };
    if(!sse::sophos::server_setup)
    {
        static sse::sophos::CryptoInitWrapper cryptoInitWrapper;
        ...
        pid_t pid = getpid();
        outfile <<dt<<" [PID: " << pid << "] "<<"     server cryptoInitWrapper ok"<<std::endl;
        outfile.close();
        sse::sophos::server_setup=true;
    }
    
    std::string pk_path = sse::sophos::storage_path_ + "/" + sse::sophos::pk_file;
    std::ifstream pk_in(pk_path.c_str());
    if(pk_in&&Sophos_server==nullptr)
    {
        // std::stringstream pk_buf;
        // pk_buf << pk_in.rdbuf();
        // Sophos_server=sse::sophos::new SophosServer(pk_buf.str());
        // std::ofstream outfile;
        // outfile.open("/opt/og/openGauss-server/sse_server_storage/server_log.txt", std::ios::app);
        // std::time_t now = std::time(nullptr);
        // char* dt = std::ctime(&now);
        // pid_t pid = getpid();
        // outfile <<dt<<" [PID: " << pid << "] "<<"     00 Sophos_server  'init or re_init' ok"<<std::endl;
        // outfile.close();
    }
    

    // sse::sophos::init_SophosServer_key();
    // outfile.open("/opt/og/openGauss-server/sse_server_storage/server_log.txt");
    // outfile <<"server key ok"<<std::endl;
    // outfile.close();
    // std::cout<<"Sophos_server_key ok"<<std::endl;
    // Sophos_server=sse::sophos::init_SophosServer();
