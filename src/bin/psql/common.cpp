...
static bool PrintQueryResults(PGresult* results)
{
    ...
    switch (PQresultStatus(results)) {
        case PGRES_TUPLES_OK:
            ...
            if(!common_sseflag.fides&&!common_delsseflag.fides)
            {
...
bool SendQuery(const char* query, bool is_print, bool print_error)
{
    ...
    if(qsfides_12 == "fides_select")
    {
        common_sseflag.fides=true;
    ...
    if(qsfides_12 == "fides_delete")
    {
        common_delsseflag.fides=true;
    }
    ...
    if (pset.fetch_count <= 0 || !is_select_command(query)) {
        ...
        {// guolv
            ...
            // fides_temp="Fides_INSERT into fides_indextable values ('";
            // for(int i=0;i<records.size();i++)
            // {
            //     if(i!=records.size()-1)
            //     {
            //         fides_temp=fides_temp+common_keyword+"','"+records[i]+"|add'),('";
            //     }
            //     else
            //     {
            //         fides_temp=fides_temp+common_keyword+"','"+records[i]+"|add');";
            //     }
            // }
            // fides_query2=fides_temp.c_str();
            // PQexec(pset.db, fides_query2);
