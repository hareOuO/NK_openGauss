...
//static Client client;
    std::cout<<"cryptoInitWrapper ok"<<std::endl;


    // Sophos_client=new sse::sophos::SophosClient;
    // std::cout<<"Sophos_client ok"<<std::endl;

    std::string filename = sse::sophos::storage_dir_path + "/" + sse::sophos::kTdpSkFile;
    std::ifstream file(filename);
    if (file) {
        Sophos_client=sse::sophos::init_SophosClient();
        std::cout<<"map count "<<Sophos_client->counter_map.size()<<"       Sophos_client ok"<<std::endl;
        file.close();
    }
    else
    {
        sse::sophos::init_SophosClient_key();
        std::cout<<"Sophos_key ok"<<std::endl;
        Sophos_client=sse::sophos::init_SophosClient();
        std::cout<<"Sophos_client ok"<<std::endl;
    }
    // sse::sophos::init_SophosClient_key();
